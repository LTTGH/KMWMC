setwd("C:/Users/LiuTT/Desktop/KM4CQG/subFun/Zeng20")
p=17
pR=6
missing=1
alpha=-1/2#-1#-1/2#-1#-1/2
beta=1
gamma=-1/10#-1/8#-1/6#-1/3#1/10#1/5#-1/2#-1/20#1/15#1/10#-1/5#-1/15#-1/8#-1/10#-1/15
delta=20#15#-5#-5#30#20#15#15#25#20#10

ZengFun20= function(seed,nlist = c(100,200)){
  library(DMwR)
  library(e1071)
  ## Linear
  ...
  misIndicTst=0
  set.seed(10000)
  dataTst=ZengDataDep20(nTst,p,pR,misIndicTst,gamma,delta,alpha,beta)
  p=dim(dataTst)[2]-3
  px=p-6 # dimension of covariates
  ...
  for(ns in nlist){
    ...
    misCovDat=ZengDataDep20(nTrain,p,pR,1,gamma,delta,alpha,beta)
    ...
    ## mean inputation
    misCovDat$x15[misCovDat$R==0]=NA
    misCovDat$x16[misCovDat$R==0]=NA
    misCovDat$x17[misCovDat$R==0]=NA
    misCovDat$x18[misCovDat$R==0]=NA
    misCovDat$x19[misCovDat$R==0]=NA
    misCovDat$x20[misCovDat$R==0]=NA
    misCovDat_meanIMP=misCovDat
    misCovDat_meanIMP$x15[is.na(misCovDat_meanIMP$x15)]=mean(misCovDat_meanIMP$x15[misCovDat_meanIMP$R==1])
    misCovDat_meanIMP$x16[is.na(misCovDat_meanIMP$x16)]=mean(misCovDat_meanIMP$x16[misCovDat_meanIMP$R==1])
    misCovDat_meanIMP$x17[is.na(misCovDat_meanIMP$x17)]=mean(misCovDat_meanIMP$x17[misCovDat_meanIMP$R==1])
    misCovDat_meanIMP$x18[is.na(misCovDat_meanIMP$x18)]=mean(misCovDat_meanIMP$x18[misCovDat_meanIMP$R==1])
    misCovDat_meanIMP$x19[is.na(misCovDat_meanIMP$x19)]=mean(misCovDat_meanIMP$x19[misCovDat_meanIMP$R==1])
    misCovDat_meanIMP$x20[is.na(misCovDat_meanIMP$x20)]=mean(misCovDat_meanIMP$x20[misCovDat_meanIMP$R==1])
    ...
    # imputeMatrix1=imputeMatrix2=matrix(0,sum(1-R),B)
    # for(i in 1:B){
    #   imputeMatrix1[,i]=t(apply(dataIC,1,IMP_MIMIMP))[,p-1]
    #   imputeMatrix2[,i]=t(apply(dataIC,1,IMP_MIMIMP))[,p]}
    # imputeValue1=apply(as.data.frame(imputeMatrix1),1,mean)
    # imputeValue2=apply(as.data.frame(imputeMatrix2),1,mean)
    # 
    # imputeData=cbind(XIC,imputeValue1,imputeValue2,YIC)
    # fullData=rbind(dataCC,imputeData)
    # fullData=as.data.frame(fullData)
    # if(p==2){cloName=c("x2","x1","Y")}else{cloName=c(paste0("x",2:p),"x1","Y")}
    # names(fullData) <- cloName
...
DRdf_400=data.frame(matrix(0,100,15))
names(DRdf_400)=c('seed','nlist','CC','WCC1','WCC2','DR1','DR2','DR3','DR4','IMP_M','IMP_KNN','IMP_MI1','IMP_MI2',
                  'CC2','Full')
DRdf_400[37,]=ZengFun20(37,c(400))
