...
alpha=-3/2
beta=4
setting1= function(seed,nlist = c(100,200,400)){
  library(DMwR)
  library(e1071)
  ## Linear
  ...
  misIndicTst=0
  set.seed(10000)
  dataTst=Sdata(nTst,misIndicTst,gamma,delta,alpha,beta)
  p=dim(dataTst)[2]-3
  px=p-1 # dimension of covariates
  ...
  for(ns in nlist){
    ...
    misCovDat=Sdata(nTrain,1,gamma,delta,alpha,beta)
    ...
    ## CC method
    discrete_psCC=tuningParamSet1(p-1,"CC","RBF",psPath0,impPath0,1)
    ...
    discrete_psWCC_CM=tuningParamSet1(p-1,"WCC","RBF",psPath1,impPath0,1)
    ...
    discrete_psWCC_MM=tuningParamSet1(p-1,"WCC","RBF",psPath0,impPath0,1)
    ...
    discrete_psDR_CMCIMP=tuningParamSet1(p-1,"DR","RBF",psPath1,impPath1,5) # PSmodel=1, Regmodel=1
    ...
    discrete_psDR_CMMIMP=tuningParamSet1(p-1,"DR","RBF",psPath1,impPath0,5) # PSmodel=1, Regmodel=0
    ...
    ## propensity score misspecified, imputation correctly specifie
    discrete_psDR_MMCIMP=tuningParamSet1(p-1,"DR","RBF",psPath0,impPath1,5)
    ...
    ## both misspecified
    discrete_psDR_MMMIMP=tuningParamSet1(p-1,"DR","RBF",psPath0,impPath0,5)
    ...
    ## mean inputation
    misCovDat$x1[misCovDat$R==0]=NA
    misCovDat_meanIMP=misCovDat
    misCovDat_meanIMP$x1[is.na(misCovDat_meanIMP$x1)]=mean(misCovDat_meanIMP$x1[misCovDat_meanIMP$R==1])
